...
> **What this file is.** A frozen, complete snapshot of `deferred-work.md` as it stood on **2026-09-07**, immediately
> before the first triage pass in the ledger's history (Epic 1 → Epic 8.2, 62 `##` sections, 284,119 bytes).
>
> **Why it exists.** Every BMad skill that touches the ledger only ever *appends* to it — `bmad-build` steps 01, 02,
> 04, `step-oneshot`, and `bmad-code-review` step-04 — so nothing was ever removed and every build paid ~71k tokens to
> load work that was long since done, superseded, or explicitly triaged away. No skill globs *this* filename, so it is
> free to keep at full length.
>
> **How to read it.** This is the ORIGINAL text, byte-for-byte, with every original section header and date intact —
> including the entries that were carried forward as OPEN into the pruned `deferred-work.md`. It is a superset, not a
> complement: an open entry appears in both files, and the live one in `deferred-work.md` is the one of record. Use
> this file to recover the history an open entry references (the struck-through closures, the corrections recorded at
> review, the measurements that are quoted elsewhere), and to audit the triage itself.
>
> **The triage verdicts** — which entries were judged OPEN, RESOLVED or SUPERSEDED and on what evidence — are recorded
> in the commit that created this file.
...
## Addendum — closed at Epic 9 scoping (2026-09-15)

Everything ABOVE this section is the 2026-09-07 snapshot, unchanged. The blocks below were removed from the live
`deferred-work.md` on 2026-09-15 when `md` scoped Epic 9; each is reproduced as it stood in the live ledger, with the
reason first. Some pre-date the snapshot and so also appear above in their older form.

### Closed: D2 closed by decision (md, 2026-09-15): the 15 Epic 7 follow-up reviews will not be run. Epic 7 is merged and in production.
...
### Closed: Already CLOSED by Story 8.3; moved out of the open ledger.

- ~~**Shopping-checkbox selector split.**~~ **CLOSED 2026-09-07 by Story 8.3 (
  `spec-8-3-check-off-an-item-by-tapping-its-row.md`).**
...
### Closed: Standing note, not work: @apollo/client patch drift is picked up by the next patch sweep.
...
### Closed: Standing convention (device-dependent ACs), not work.
...
### Closed: Standing convention (probe scripts restore state and clean up), not work.
...
### Closed: Standing convention (cite changelogs for transitive majors), not work.
...
- source_spec: `spec-7-12-graphql-kotlin-9-to-10-with-kotlin.md`
  ...
  breaking changes the way `graphql-java` was handled, or state explicitly that the observable-surface captures are
  being
...
### Closed: Moot: the bmad-dev-auto skill was retired (replaced by bmad-build / bmad-build-auto). D3 closed with it.
...
### Closed: Already CLOSED, delivered by Story 8.5.
...
### Closed: DECIDED by Story 8.4; its open follow-up (name-keyed testids) stays live in the ledger.
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-1-move-the-mobile-gate-to-the-width-people-actually-use.md`
  ...
  **Decided by Story 8.4 (`spec-8-4-one-filter-and-search-on-both-list-screens.md`). Decision: KEEP the
  ...
  `item-row-<name>` throughout, and so does every FR61 assertion on the shopping side (`shopping-item-<name>`). **Why
  keep:** Epic 8's own contract keys BOTH surfaces by name (that is what makes a shopping-side and a
...
### Closed: DECIDED by Story 8.5 (no Uncategorized filter option).
...
### Closed: DECIDED by Story 8.6 (a stale rename recreating a removed category is not a bug).
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-6-rename-a-category-instead-of-destroying-it.md`
  ...
  `CategoryUpdates` subscription. **Why it is left alone:** the alternative is a client-side existence check before
  every save, which is a race
...
### Closed: NOTE, not work; the design-contract pointer was folded into the Story 8.7 section intro.
...
## Addendum — second pass of the 2026-09-15 triage

The blocks below were removed from the live `deferred-work.md` later on 2026-09-15, when `md` went through every
entry not routed to Epic 9. Each is CLOSED BY DECISION: it will not be worked, and no reason beyond the decision
is recorded. Blocks are reproduced as they stood in the live ledger, grouped under the section they came from;
where a whole section emptied, its intro paragraphs came with it.

### Closed by decision (md, 2026-09-15) — from: Epic 5 close-out — carried forward (2026-07-28)

- **FR42 (one-timer) / FR43 (recurring) item UI deferred by epic design.** Backend support is complete and live,
  including the hourly scheduler; the UI affordances were intentionally postponed. `AddItemDialog` sends
  `recurring: null`. UI-only work against a frozen, tested contract whenever it is picked up. **Its recorded
  prerequisite is discharged 2026-08-10 by Story 7.4:** BUG-E6-2 (an edit wiping `checkedAt` and so
  permanently hiding the item from the scheduler) is fixed, so shipping the lifecycle control no longer ships a way to
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.1 — E2E suite inside the frontend quality gates (2026-08-07)

- **Type-aware linting is not enabled, so `@typescript-eslint/no-floating-promises` does not run — an un-awaited
  Playwright assertion still ships undetected.** This is the single highest-value remaining addition to the frontend
  static gate and the one rule class that would catch the canonical "assertion that never ran" defect
  (`expect(locator).toBeVisible()` without `await`). Story 7.1 audited all 2,474 lines of `e2e/` and found **zero**
  instances today, but nothing in the current rule set would catch a future one. **Why deferred (Story 7.1 Decision
  9):**
  `bp_front/eslint.config.mjs` extends `tseslint.configs.recommended`, not `recommendedTypeChecked`, and
...
- source_spec: `_bmad-output/implementation-artifacts/spec-7-1-e2e-suite-inside-frontend-quality-gates.md`
  ...
  evidence: two consecutive `npx tsc -b --verbose` runs both print `Project 'tsconfig.e2e.json' is out of date because
  output file 'e2e/account.spec.js' does not exist` and rebuild everything. With `noEmit` and no `composite`, the
  `.tsbuildinfo` is written (337 bytes — not a real program graph) but never satisfies the up-to-date check.
  Pre-existing
  for the two original projects; Story 7.1 added a third without observing it. Low consequence today (full build ~2s).
  The `composite: true` + `noEmit: true` combination is accepted on TypeScript 6.0.3 and is the obvious thing to test —
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.2 — shared E2E support module (2026-08-08)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.3 — delete the
`registrationEnabled` race (2026-08-08)
...
- **A red `chromium` or `mobile` now costs the FR20/FR21 toggle coverage entirely — measured, not assumed.** With one
  ...
  `--no-deps`, so registration is still enabled). **The real fix candidate is `testProject.teardown`, and it was never
  evaluated.** The config comment written by this
  story claimed `dependencies` is "the only Playwright construct that orders work across projects" — that is false.
  `teardown` also orders across projects, and unlike `dependencies` it still runs when the run is red, which would
  dissolve this entire cost rather than mitigate it. The alternatives that *were* weighed were `mode: 'serial'`
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.4 — an item edit merges the stored item (2026-08-10)
...
- **`saveItem`'s create branch still accepts a `category` that belongs to no list** (`md`'s ruling A: the check is
  ...
  **and** gone from Mongo, takes the **create** branch — the branch this ruling deliberately leaves unguarded — and
  resurrects the item carrying the now-deleted category. That composes the two holes this story knowingly left open (the
  unguarded create branch here, and BUG-E6-3a's resurrection below) into BUG-E6-3b's state through a path a normal
  user can walk. **Price this entry accordingly: it is not "direct API callers only".**
...
- **BUG-E6-3a — a save against a hard-deleted id resurrects the item. Severity-downgraded by Story 7.4, NOT fixed (
  ...
  `checkedAt` null, and the result is visible and removable through the UI. Before the merge the same write
  reconstructed
  ...
  not a data-integrity failure. **The real fix is soft-delete tombstones that the scheduler owns:** make `deleteItem`
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.5 — home resolution and the inert home link (2026-08-11)
...
- **Nothing mechanically prevents a third `createdAt` sort site from reappearing with `localeCompare`.** Both existing
  ...
  rediscovery is one more ~1-in-1000 wrong-list bug. **STILL OPEN after Story 8.5 (2026-09-08), and the routing above is
  half-discharged.** 8.5 landed the *module* half
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.6 — backend safety fixes riding the same unfreeze (2026-08-12)

- **`@Volatile` fixes the visibility half of the lazy-sync race and leaves the check-then-act window open.** All three
  `sync()` guards are still `if (synced.not()) { <suspending repository.getAll()> ; synced = true }`, so two coroutines
  arriving before either finishes can both read `false` and both run the startup load. What the fix buys is that once
  one of them writes `true`, every other thread reliably *sees* `true`. **The double load is NOT merely wasted I/O.**
  The two passes are only idempotent if no write interleaves, and one
  ...
  `storage.save`/`storage.delete`, so the scheduler races the first HTTP request on `synced`. **Proposed fix:** a
  `kotlinx.coroutines.sync.Mutex` held across the load (`synchronized` is not an option — the body
...
- `771-776` — `ListStorage.rename` is still not atomic (in-memory updated before the Mongo write,
  `ListStorage.kt:41-47`),
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.7 — minor and patch dependency sweep (2026-08-12)
...
- source_spec: `spec-7-7-minor-and-patch-dependency-sweep.md`
  summary: the Gradle wrapper is held at 9.6.1 while 9.7.0 is released. **Named blocking symptom:**
  `bp_back/Dockerfile:1`
  ...
  `gradle/wrapper/gradle-wrapper.properties` and `bp_back/Dockerfile:1` still read 9.6.1, so the pair is coherent and
  the
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.10 — TypeScript 6 → 7 (2026-08-15)
...
- source_spec: `spec-7-10-typescript-6-to-7.md`
  ...
  **the peer conflict does not stop you, the runtime does.** The blocking peer is declared by exactly 8 packages
  (`typescript-eslint` plus seven `@typescript-eslint/*`); the other `typescript` peers in the lockfile (`ts-api-utils`,
  two `cosmiconfig` copies) are open-ended and do not block TS 7.
  evidence, measurement 3 — the lint gate, verbatim from `npm run lint` with TS 7.0.2 installed, **exit code 2**:
  ```
  typescript-eslint does not support TS 7.0.
  ...
  ...
  be waved through with a range override. **The finding worth keeping — this codebase is already TS-7-clean.** With TS
  7.0.2 installed:
  ...
  built under TS 7** — `docker compose build bp_front` is an unexercised path. **Rejected escape hatch: the sanctioned
  side-by-side / dual-TypeScript install** (alias TS 6 in so `typescript` still
  ...
  *downgrade* the linting compiler rests on `@typescript/typescript6`'s published `latest` being `6.0.2`, a version
  ...
  co-equal measured grounds. **And the hold imposes an inverse constraint on any `typescript-eslint` move:** whatever
  version lands must still
  declare a `typescript` peer admitting the held **6.0.3** — i.e. the `<6.1.0` line. 31 of the swept releases declare
  `>=4.8.4 <6.0.0`, which does *not* admit 6.0.3, so an ESLint-driven move of `typescript-eslint` could re-open the
  same conflict from the other side. **Re-check trigger, concrete.**
  Watch <https://github.com/typescript-eslint/typescript-eslint/issues/10940>. (Read
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.11 — ESLint 9 → 10 (2026-08-16)
...
- source_spec: `spec-7-11-eslint-9-to-10.md`
  ...
  evidence: `eslint@9.39.5` engines were `^18.18.0 || ^20.9.0 || >=21.1.0`; `eslint@10.8.1` is
  `^20.19.0 || ^22.13.0 || >=24` (both read from the lockfiles). `vite@8` admits `^20.19.0 || >=22.12.0`. So Node
  22.12.x and 23.x pass every other gate and cannot run `npm run lint`, and `npm` only emits an `EBADENGINE`
  **warning**.
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.13 — `graphql` 16 → 17 (2026-08-19)
...
- source_spec: `spec-7-13-graphql-16-to-17.md`
  ...
  `peer graphql: "^0.11.0 || … || ^16.0.0"` — no `^17` — and it is a **hard dependency** of
...
- source_spec: `spec-7-13-graphql-16-to-17.md`
  summary: Nothing fences graphql 17's narrowed Node floor — no `engines` field, no `.nvmrc`, no `.node-version`.
  evidence: graphql 17.0.2 declares `engines.node: "^22.0.0 || ^24.0.0 || ^25.0.0 || >=26.0.0"`, where 16.14.2 accepted
  anything `>=`. The current paths are fine (`mise.toml` pins the major `26`; `bp_front/Dockerfile` is
  `node:26-alpine`),
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 7.14 — installable PWA (2026-08-20)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 7-14-installable-pwa (2026-08-20)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 7-12-graphql-kotlin-9-to-10-with-kotlin (2026-08-16)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 7-10-typescript-6-to-7 (2026-08-15)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 7-8-7-9-types-node-26-and-vite-8 (2026-08-13)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 7-7-minor-and-patch-dependency-sweep (2026-08-12)
...
- source_spec: `spec-7-7-minor-and-patch-dependency-sweep.md`
  ...
  evidence: `playwright.config.ts` spreads `devices['Pixel 7']` with no explicit `viewport` override. A descriptor
  change would silently alter what "the mandatory mobile gate" actually tests, and the whole suite would stay green
  either way. **Sharper since Story 8.1:** the project now renders at the 320px floor (`PIXEL_7_AT_FLOOR`), so the
  descriptor is a *base* the epic's geometry gate is measured against — a silent shift in
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 7-6-backend-safety-fixes (2026-08-12)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 7-4-item-edit-merges-stored-item (2026-08-10)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 7-3-delete-registrationenabled-race (2026-08-08)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 7-2-shared-e2e-support-module (2026-08-08)

- source_spec: `_bmad-output/implementation-artifacts/spec-7-2-shared-e2e-support-module.md`
  summary: `PASSWORD` lives in `support/ui.ts`, so any pure-API consumer must import the runner-importing module to get
  a
  credential — the exact coupling the two-file split exists to prevent.
  evidence: all seven `loginApi(x, PASSWORD)` call sites pair `./support/api`'s function with `./support/ui`'s constant,
...
- source_spec: `_bmad-output/implementation-artifacts/spec-7-2-shared-e2e-support-module.md`
  ...
  files must not match `*.spec.ts`" and "imports must be relative" are enforced by nothing; a one-line
  `import {expect} from '@playwright/test'` added to `api.ts` passes both gates today. Two cheap enforcements exist and
  were **blocked by Story 7.2's own no-config-change boundary**, not judged unnecessary: an ESLint
  `no-restricted-imports`
  entry scoped to `files: ['e2e/support/api.ts']`, and an explicit `testMatch`/`testIgnore` in `playwright.config.ts`
  (which would also convert the documented `support/helpers.spec.ts` trap from a warning into an impossibility). **This
  is one of the three items the "gate the E2E invariants" story named under Story 7.3 would discharge together.**
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of epic-7-context (2026-08-21)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 4-8-frontend-lists-tab-list-management-bpavatar (2026-05-25)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 4-3-list-sharing-backend-pending-invites-member-management (2026-05-22)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 4-2-websocket-auth-per-list-subscription-scoping (2026-05-22)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 4-1-list-entity-backend-crud-authorization-migration (2026-05-22)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 3-1-deferred-work-triage-high-priority-fixes (2026-05-15)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 2-4-registration-toggle-ui-adaptive-login-screen (2026-05-15)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 2-2-admin-user-management-backend (2026-05-15)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 2-1-applicationconfig-entity-registration-toggle-backend (2026-05-14)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 1-2-login-token-system-session-security-backend (2026-05-08)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 1-1-user-entity-registration-backend (2026-05-08)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 5-1-foundation-vite-mui-caddy-apollo-shell (2026-07-14)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 5-2-authentication (2026-07-15)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of ssl-termination-single-entrypoint (2026-07-16)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of story-5.3 (2026-07-17)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: planning of 5-5-lists-management (2026-07-21)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 5-7-sharing-and-membership (2026-07-23)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 8.2 — a long name and a full header fit on a narrow phone (2026-09-05)
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-2-a-long-name-and-a-full-header-fit-on-a-narrow-phone.md`
  ...
  the other two are `ListsPage.tsx:195` and `AdminPage.tsx:200`, in the entry below — and it still truncates a long
  username at the 320px floor. AC4 required it be measured rather than pass silently; it is **left as shipped**, with
  the measurement here. (Corrected twice. The Story 8.2 review called this "the FOURTH instance", which read as an
  exhaustive audit; the
  correction then said "at least FOUR … see the entry below for the other three" while that entry names TWO. Verified
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of 8-1-move-the-mobile-gate-to-the-width-people-actually-use (2026-09-05)
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-4-one-filter-and-search-on-both-list-screens.md`
  ...
  evidence: Carried forward from the Story 8.1-review DECIDED entry (moved to the archive 2026-09-15) with its decision
  applied: the testids stay name-keyed for now, and the non-uniqueness is
  ...
  ALL of these surfaces together and visit each of the ~35 call sites; a partial re-key would leave the two screens
  keyed
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: code review of spec-8-2-a-long-name-and-a-full-header-fit-on-a-narrow-phone (2026-09-06)
...
### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 8.6 — rename a category instead of destroying it (2026-09-08)
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-6-rename-a-category-instead-of-destroying-it.md`
  ...
  reachable before this story. Both list surfaces key their rows by NAME (`category-row-${group.name}`,
  `shopping-group-${group.name}`), so two same-named categories give two rows with the
  SAME testid — which is why `e2e/order.spec.ts` (Story 8.5) asserts the duplicate-name ordering case against the
  exported `groupItemsByCategory` rather than through the DOM. **Why 8.6 did not fix it:** category names are not
  unique — not in the schema, not in `CategoryService`, not in the
  ...
  Epic 8. **Shape of the real fix:** a uniqueness rule in `CategoryService.saveCategory` (per list, case-folded),
  surfaced by
  both dialogs through the inline `*-category-error` alert they already render for a rejected save.

### Closed by decision (md, 2026-09-15) — from: Deferred from: Story 8.7 — write down the design this app actually has (2026-09-09)
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-7-write-down-the-design-this-app-actually-has.md`
  ...
  `ListShoppingPage.tsx:167` (item name), `:406` (page header), `:485` (group heading). **What exactly is superseded,
  quoted in full** so this is not read as catching a bad grep. The Story 8.2 entry
  ...
  "three". **The old entry's anchor has drifted again, for the second time.** `ListShoppingPage.tsx:215` was itself a
...
- source_spec: `_bmad-output/implementation-artifacts/spec-8-7-write-down-the-design-this-app-actually-has.md`
  ...
  follows.
  evidence: All measured 2026-09-09 at `3af2d575e852ca186467c67a051e5ddc77a6fe6d`. **(1) Item-name truncation.**
  `/lists/:id` clamps to two lines then ellipsises — `display: '-webkit-box'`,
  ...
  design from drift. **(2) Row interactivity.** Three implementations: a `ListItemButton` covering the row body on
  `/lists`
  ...
  (FR60, and AR-E8-8a closes it as an extension surface); the first two have simply never been compared. **(3) Alert
  idiom deviations.** The convention is an `<Alert severity="error" role="alert">` rendered in the
