...
- [x] Task 1: Add GraphQL operations for user admin (AC: 1, 4, 5, 6)
    - [x] Create `bp_front/src/lib/user/Queries.tsx` with `getUsersQuery`, `createUserMutation`, `deleteUserMutation`,
...
- [x] Task 2: Create `ConfirmDialog` component (AC: 4, 5, 6)
    - [x] Create `bp_front/src/app/admin/ConfirmDialog.tsx`
    - [x] Props: `open`, `title`, `message`, `confirmLabel`, `confirmColor` (`"error" | "primary"`), `onConfirm`,
      `onCancel`, optional `children`
    - [x] `maxWidth="xs"`, initial focus on Cancel, confirm button shows `CircularProgress` and is disabled during async
      operation
    - [x] Escape closes dialog (MUI Dialog default — no extra code needed)

- [x] Task 3: Create admin route guard (AC: 7)
    - [x] Create `bp_front/src/app/admin/layout.tsx` — client component using `useAuth()` to check `role === 'admin'`;
      redirect to `/` if not admin or if loading is done and role is not admin

- [x] Task 4: Create `AdminUsersPage` (AC: 1–6)
    - [x] Create `bp_front/src/app/admin/users/page.tsx`
    - [x] Use `useQuery(getUsersQuery)` with Apollo Client; derive list from `data.users`
    - [x] Render `CircularProgress` centred in table area while `loading` is true; keep table header visible
    - [x] Empty state: single row with muted `Typography` "No users yet. Create the first one."
    - [x] "Create user" `Button` above table; opens create dialog
    - [x] Per-row: reset-password `IconButton` (title="Reset password") and delete `IconButton` (title="Delete user")
    - [x] `useMutation` for `createUserMutation` — optimistic update: append returned user to `data.users`
    - [x] `useMutation` for `deleteUserMutation` — optimistic update: remove user from `data.users` by id
    - [x] `useMutation` for `resetUserPasswordMutation` — no UI update needed; just close dialog

- [x] Task 5: Add "User Management" nav item to Navigation (AC: 8)
    - [x] Edit `bp_front/src/app/Navigation.tsx` — add a `MenuItem` linking to `/admin/users`, rendered only when
      `role === 'admin'`

- [x] Task 6: Verify with lint
    - [x] From `bp_front/`: `npm run lint` — no new lint errors (no lint script; TypeScript check passes clean)
...
- Apollo hooks import must be from `@apollo/client/react`, not `@apollo/client` (project convention)
- No `npm run lint` script exists; TypeScript check (`tsc --noEmit`) used instead — passes clean

### Completion Notes List

- Task 1: `Queries.tsx` created with all 4 GQL operations; codegen run by user (generated types confirmed present)
- Task 2: `ConfirmDialog.tsx` — MUI Dialog with internal loading state, `autoFocus` on Cancel, async `onConfirm`
- Task 3: `admin/layout.tsx` — mirrors `RouteGuard.tsx` pattern; redirects non-admin to `/` after auth resolves
- Task 4: `admin/users/page.tsx` — full CRUD UI with Apollo cache updates for create/delete; 3 ConfirmDialog instances
- Task 5: `Navigation.tsx` updated — `role === 'admin'` guard wraps "User Management" MenuItem
- TypeScript strict mode: cache update callbacks typed via `GetUsersQuery['users'][number]`

### File List

- bp_front/src/lib/user/Queries.tsx (new)
- bp_front/src/app/admin/ConfirmDialog.tsx (new)
- bp_front/src/app/admin/layout.tsx (new)
- bp_front/src/app/admin/users/page.tsx (new)
- bp_front/src/app/Navigation.tsx (modified)
- bp_front/src/__generated__/graphql.ts (auto-generated by user)
- bp_front/src/__generated__/gql.ts (auto-generated by user)
- bp_front/src/__generated__/fragment-masking.ts (auto-generated by user)

### Review Findings

- [x] [Review][Patch] All mutation handlers close dialogs unconditionally on failure — `handleCreate`/`handleDelete`/
  `handleReset` run state-clearing (setCreateOpen/setDeleteTarget/setResetTarget) unconditionally after
  `await mutation(...)` and `ConfirmDialog.handleConfirm` has no catch block; a GraphQL error or network failure
  silently closes the dialog with no user feedback [page.tsx] — fixed: added catch+error state to ConfirmDialog;
  handlers already correctly skip close on throw
- [x] [Review][Patch] No client-side validation — create dialog submits with empty username/password; reset dialog
  submits with empty new password; no disabled state on Confirm button when required fields are blank [page.tsx] —
  fixed: added `confirmDisabled` prop to ConfirmDialog; passed trim() guards for create and reset dialogs
- [x] [Review][Patch] Cancel button remains interactive during in-flight mutation — `disabled={loading}` in
  ConfirmDialog only blocks Confirm; Cancel can fire and reset parent state while the mutation promise is still in
  flight [ConfirmDialog.tsx] — fixed: added `disabled={loading}` to Cancel button
- [x] [Review][Patch] Relative `../ConfirmDialog` import violates `@/` import rule — should be
  `import ConfirmDialog from '@/app/admin/ConfirmDialog'` [page.tsx:28] — fixed
- [x] [Review][Patch] Inline mutation response shape types instead of generated types — `update` callbacks use
  `{data?: {createUser: GetUsersQuery['users'][number]} | null}` instead of importing `CreateUserMutation`/
  `DeleteUserMutation` from `@/__generated__/graphql` [page.tsx:41,53] — fixed: removed explicit annotations, relying on
  typed document node inference
- [x] [Review][Defer] Client-side-only admin guard — `getUsersQuery` fires before `layout.tsx` redirect executes in
  `useEffect`; a logged-in non-admin receives user list data before redirect — pre-existing Next.js App Router
  client-auth limitation, not introduced by this story

## Change Log

- 2026-05-15: Implemented story 2-3 — admin user management UI; all 6 tasks complete, TypeScript clean
